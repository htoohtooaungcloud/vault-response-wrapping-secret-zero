...
resource "vault_approle_auth_backend_role" "trusted_entity_approle_role" {
  ...
  token_max_ttl         = 4320
  token_bound_cidrs     = ["172.20.0.3/32"] # Could it be Jenkins Server's IP Address or Load-Balancer's IP Address as LIST
  secret_id_bound_cidrs = ["172.20.0.3/32"] # Could it be Jenkins Server's IP Address or Load-Balancer's IP Address as LIST

...
vault write auth/approle/role/container-registry  \
    ...
    token_bound_cidrs="172.19.0.4/32" \
    secret_id_bound_cidrs="172.19.0.4/32" \
...
```
docker exec -it hellocloud-jenkins-1 bash
```
### Verify whether `Vault` Binary is executable.
```
vault version
Vault v1.18.0 (77f26ba561a4b6b1ccd5071b8624cefef7a72e84), built 2024-10-08T09:12:52Z
```

### Set `Vault` Env Var in Jenkins container 
```
export VAULT_ADDR=https://vault.htoohtoo.cloud:8443
```
> [!NOTE]
> At this point, we're  not going to set Env Var for `Vault` Token.

### Log in to `Vault` using AppRole auth method, Generate the `Trusted-Entity` Rold-ID and Secret-ID
- The output should only display the `Token` that will be generated by the `Vault` after authenticated 
```
...
- Prefer install harbor on separate **VM** and generate SSL/TLS certificate using `certbot` for this project.
